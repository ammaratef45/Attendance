...
class DBProvider {
  ...
  static const String _userTableV2 = 'CREATE TABLE USER ('
      'id INTEGER PRIMARY KEY,'
      'nativeName TEXT'
      'phone TEXT'
      'apiSynced int'
      ')';
  static const String _scanTableV2 = 'CREATE TABLE Scan ('
      'id INTEGER PRIMARY KEY,'
      'key TEXT,'
      'classKey TEXT,'
      'admin TEXT,'
  ...
  Future<List<Scan>> getAllScans() async {
    ...
    final List<Scan> list = res.isNotEmpty
  ...
  Future<int> updateScan(Scan scan) async {
    ...
    final int res =
        await db.update('Scan', scan.toMap(), where: 'id = ?', whereArgs: args);
    return res;
  }
  ...
  Future<int> saveUser(User user) async {
    ...
    args['apiSynced'] = 0;
    final int res =
        await db.update('User', args, where: 'id = ?', whereArgs: <dynamic>[1]);
    return res;
  }
