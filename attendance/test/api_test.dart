...
Future<String> token() async {
  final ProcessResult p = await Process.run('/usr/bin/python',
      //'python',
      <String>['firebase_token_generator.py', 'GUfzhtGu1vVFJaYIvxi1yIa49Oy1']);
...
void main() {
  ...
  group('session calls', () {
    ...
    test('leave success', () async {
      ...
      expect(res, '{"success":"true","message":"updated","data":null}');
    });
    ...
    test('leave fails due to wrong token', () async {
      try {
        ...
        expect(
            e,
            contains('status code is not 200\n'
                '{"success":"false",'));
      }
    });
  }, skip: true);
